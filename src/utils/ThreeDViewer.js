...
function ThreeDViewer({ currentIndex, models, onNavigate }) {
  ...
  const adjustModelScale = (model) => {
    ...
    let scale;
    if (screenWidth <= 350) scale = 0.3;
    else if (screenWidth <= 475) scale = 0.4;
    else if (screenWidth <= 560) scale = 0.5;
    else if (screenWidth <= 780) scale = 0.5;
    else if (screenWidth <= 860) scale = 0.6;
    else if (screenWidth <= 960) scale = 0.65;
    else if (screenWidth <= 1024) scale = 0.7;
    else if (screenWidth <= 1100) scale = 0.75;
    else if (screenWidth <= 1280) scale = 0.8;
    else if (screenWidth <= 1450) scale = 0.75;
    else if (screenWidth <= 1600) scale = 0.8;
    else scale = 0.9;
  ...
  useEffect(() => {
    ...
    scene.add(new THREE.DirectionalLight(0xffffff, 1).position.set(0, 1, 2));
    scene.add(new THREE.AmbientLight(0xffffff, 0.6));
    scene.add(new THREE.HemisphereLight(0xffffff, 0x444444, 2).position.set(0, 5000, 0));

    const loader = new GLTFLoader();

    const loadModel = (index) => {
      if (model) scene.remove(modelGroup);
